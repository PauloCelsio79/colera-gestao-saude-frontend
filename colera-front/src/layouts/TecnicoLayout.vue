  ...
  <div class="min-h-screen bg-secondary-900">
    ...
    <aside class="fixed inset-y-0 left-0 w-64 bg-secondary-800 text-white transition-transform duration-300 transform">
      <div class="flex items-center justify-center h-20 bg-secondary-900">
        <h1 class="text-2xl font-bold text-primary-500">Painel Técnico</h1>
      </div>
      <nav class="mt-6 px-4">
        <router-link 
...
import { TruckIcon, MapIcon, WrenchScrewdriverIcon } from '@heroicons/vue/24/outline'
import api from '@/api'
...
const menuItems = [
  { name: 'Gestão de Ambulâncias', path: '/dashboard/tecnico/ambulancias', icon: TruckIcon },
  { name: 'Coordenação de Transportes', path: '/dashboard/tecnico/coordenacao', icon: MapIcon },
  { name: 'Manutenção e Logística', path: '/dashboard/tecnico/manutencao', icon: WrenchScrewdriverIcon },
]

const currentPageTitle = computed(() => {
  const currentItem = menuItems.find(item => item.path === route.path)
  return currentItem ? currentItem.name : 'Painel Técnico'
})
